...
public abstract class UDPMessenger {
    ...
    protected static String TAG = "UDPMessenger";
     ...
     * @param tag
     *            a valid string, used to filter the UDP broadcast messages (in and out). It can't be null or 0-characters long.
     * @param port
     *            the port to multicast to. Must be between 1025 and 49151 (inclusive)
     * @param connectionPort
     *            the port to get the connection back. Must be between 1025 and 49151
     */
...
//        }
    ...
    public boolean sendMessage(String message, String ip, int port) throws IllegalArgumentException, UnsupportedEncodingException,
    ...
    public boolean sendMessage(InetAddress inetAddr, int port, byte[] data) {
        if (DBG)
            Log.d(TAG, "sendMessage. [data size=" + data.length + ", inetAddr=" + inetAddr + ", port=" + port);
                ...
                for (InterfaceAddress ifa : interfaceAddresses) {
                    if (DBG) {
                        if ((ifa.getAddress() instanceof Inet4Address)) {
                            ...
                            Log.d(TAG, "prefixNet=" + bytesToHex(ByteBuffer.allocate(4).putInt(prefixNet).array()));
                            ...
                            Log.d(TAG, "is ipv4=" + (ifa.getAddress() instanceof Inet4Address) + ", leng=" + address.length);
