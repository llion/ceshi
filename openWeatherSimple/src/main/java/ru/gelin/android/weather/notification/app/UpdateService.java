...
import static ru.gelin.android.weather.notification.app.PreferenceKeysApp.LOCATION_DEFAULT;
import static ru.gelin.android.weather.notification.app.PreferenceKeysApp.LOCATION_TYPE;
import static ru.gelin.android.weather.notification.app.PreferenceKeysApp.REFRESH_INTERVAL;
import static ru.gelin.android.weather.notification.app.PreferenceKeysApp.REFRESH_INTERVAL_DEFAULT;
...
import ru.gelin.android.weather.notification.skin.impl.PreferenceKeys;
...
import com.example.openweathersimple.R;

/**
 * Service to update weather. Just start it. The new weather values will be wrote to SharedPreferences (use
 * {@link ru.gelin.android.weather.notification.WeatherStorage} to extract them).
 */
public class UpdateService extends Service implements Runnable {
    ...
    /** Force flag */
    boolean force = false;
    AppUtils.WeatherParcelable wp = null;
    /** Queried location */
    Location location;
    /** Updated weather */
    Weather weather;
    ...
    public void onStart(Intent intent, int startId) {
        ...
        synchronized (this) {
            ...
            if (intent != null) {
                ...
                this.wp = intent.getParcelableExtra(AppUtils.EXTRA_WP);
            }
        }
        ...
        synchronized (staticLock) {
            if (threadRunning) {
                if (DBG)
                    Log.i(TAG, "onStart. threadRunning intent, startId, Thread=" + Thread.currentThread());
    ...
    void skipUpdate(WeatherStorage storage, String logMessage) {
        if (DBG)
            Log.i(TAG, "skipUpdate. storage, logMessage=" + logMessage);
        stopSelf();
        storage.updateTime();
    ...
    public void run() {
        ...
        if (DBG)
            Log.i(TAG, "run. locationType=" + locationType);
        ...
        if (LocationType.LOCATION_MANUAL.equals(locationType)) {
            ...
            location = new NameOpenWeatherMapLocation(this, preferences.getString(PreferenceKeys.LOCATION, LOCATION_DEFAULT), null);
            if (DBG)
                Log.i(TAG, "run. , preferences LOCATION=" + preferences.getString(PreferenceKeys.LOCATION, LOCATION_DEFAULT));
        } else {
            ...
            case SUCCESS:
                synchronized (UpdateService.this) {
                    ...
                    if (verbose && weather.isEmpty()) {
                        Toast.makeText(UpdateService.this,
                                getString(R.string.weather_update_empty),
                                Toast.LENGTH_LONG).show();
                    }
                }
                break;
    ...
    /**
     * Check availability of network connections. Returns true if any network connection is available.
     */
    ...
    LocationType getLocationType() {
        ...
        return LocationType.valueOf(preferences.getString(
                LOCATION_TYPE, LocationType.LOCATION_MANUAL.toString()));
    ...
    Location createSearchLocation(String query) {
        LocationManager manager = (LocationManager) getSystemService(Context.LOCATION_SERVICE);
        if (manager == null) {
            return new NameOpenWeatherMapLocation(this, query, null);
        }
        android.location.Location androidLocation =
                manager.getLastKnownLocation(LocationManager.NETWORK_PROVIDER);
        return new NameOpenWeatherMapLocation(this, query, androidLocation);
    }
     ...
     * 
     * @param intent
     *            to wrap into the pending intent or null
     */
