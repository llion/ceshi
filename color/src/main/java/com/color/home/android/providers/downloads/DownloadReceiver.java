...
public class DownloadReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, final Intent intent) {
        if (DBG)
            Log.i(TAG, "onReceive. action=" + intent.getAction() + ", intent=" + intent);
        ...
        if (action.equals(Intent.ACTION_BOOT_COMPLETED)) {
            if (Constants.LOGVV) {
                Log.v(Constants.TAG, "Received broadcast intent for " + Intent.ACTION_BOOT_COMPLETED);
            }
            startService(context);
        } else if (action.equals(Intent.ACTION_MEDIA_MOUNTED)) {
            if (Constants.LOGVV) {
                Log.v(Constants.TAG, "Received broadcast intent for " + Intent.ACTION_MEDIA_MOUNTED);
            }
            startService(context);
    ...
    private void handleNotificationBroadcast(Context context, Intent intent) {
        ...
        if (Constants.ACTION_LIST.equals(action)) {
            final long[] ids = intent.getLongArrayExtra(CLDownloadManager.EXTRA_NOTIFICATION_CLICK_DOWNLOAD_IDS);
    ...
    private void hideNotification(Context context, long id) {
        ...
        //
        // final Uri uri =
        // ContentUris.withAppendedId(Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI,
        // id);
    ...
    private void openDownload(Context context, long id) {
        // if (!OpenHelper.startViewIntent(context, id,
    ...
    private void sendNotificationClickedIntent(Context context, long[] ids) {
        ...
        final Uri uri = ContentUris.withAppendedId(Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI, ids[0]);
        ...
        if (isPublicApi) {
            appIntent = new Intent(CLDownloadManager.ACTION_NOTIFICATION_CLICKED);
            appIntent.setPackage(packageName);
            appIntent.putExtra(CLDownloadManager.EXTRA_NOTIFICATION_CLICK_DOWNLOAD_IDS, ids);
