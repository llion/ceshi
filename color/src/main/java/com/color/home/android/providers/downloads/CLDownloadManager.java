 ...
 * particular destination file. The download manager will conduct the download in the background, taking care of HTTP interactions and
 * retrying downloads after failures or across connectivity changes and system reboots.
 * 
 * Instances of this class should be obtained through {@link android.content.Context#getSystemService(String)} by passing
 * {@link android.content.Context#DOWNLOAD_SERVICE}.
 * 
 * Apps that request downloads through this API should register a broadcast receiver for {@link #ACTION_NOTIFICATION_CLICKED} to
 * appropriately handle when the user clicks on a running download in a notification or from the downloads UI.
 * 
 * Note that the application must have the {@link android.Manifest.permission#INTERNET} permission to use this class.
 */
public class CLDownloadManager {
    // never public, so that another class won't be messed up.
    private final static String TAG = "CLDownloadManager";
    private static final boolean DBG = false;

    /**
     * An identifier for a particular download, unique across the system. Clients use this ID to make subsequent calls related to the
     * download.
     */
    ...
    /**
     * Internet Media Type of the downloaded file. If no value is provided upon creation, this will initially be null and will be filled in
    ...
    /**
     * Uri where downloaded file will be stored. If a destination is supplied by client, that URI will be used here. Otherwise, the value
     * will initially be null and will be filled in with a generated URI once the download has started.
     */
     ...
     * 
     * When {@link #COLUMN_STATUS} is {@link #STATUS_FAILED}, this indicates the type of error that occurred. If an HTTP error occurred,
     * this will hold the HTTP status code as defined in RFC 2616. Otherwise, it will hold one of the ERROR_* constants.
     * 
     * When {@link #COLUMN_STATUS} is {@link #STATUS_PAUSED}, this indicates why the download is paused. It will hold one of the PAUSED_*
     * constants.
     * 
    ...
    /**
     * Timestamp when the download was last modified, in {@link System#currentTimeMillis System.currentTimeMillis()} (wall clock time in
     * UTC).
     */
    public final static String COLUMN_LAST_MODIFIED_TIMESTAMP = "last_modified_timestamp";

    /**
     * The URI to the corresponding entry in MediaProvider for this downloaded entry. It is used to delete the entries from MediaProvider
    ...
    /**
     * Value of {@link #COLUMN_REASON} when a storage issue arises which doesn't fit under any other error code. Use the more specific
    ...
    /**
     * Value of {@link #COLUMN_REASON} when the requested destination file already exists (the download manager will not overwrite an
     * existing file).
     */
    ...
    /**
     * Value of {@link #COLUMN_REASON} when the download is paused because some network error occurred and the download manager is waiting
    ...
    /**
     * Value of {@link #COLUMN_REASON} when the download exceeds a size limit for downloads over the mobile network and the download manager
    ...
    /**
     * Intent extra included with {@link #ACTION_VIEW_DOWNLOADS} to start DownloadApp in sort-by-size mode.
     */
    public final static String INTENT_EXTRAS_SORT_BY_SIZE = "color.android.app.CLDownloadManager.extra_sortBySize";

    /**
     * Intent extra included with {@link #ACTION_DOWNLOAD_COMPLETE} intents, indicating the ID (as a long) of the download that just
     * completed.
     */
    public static final String EXTRA_DOWNLOAD_ID = "extra_download_id";

    /**
     * When clicks on multiple notifications are received, the following provides an array of download ids corresponding to the download
     * notification that was clicked. It can be retrieved by the receiver of this Intent using
     ...
     * @hide
     */
    public static final String[] UNDERLYING_COLUMNS = new String[] { Downloads.Impl._ID,
            Downloads.Impl._DATA + " AS " + COLUMN_LOCAL_FILENAME, Downloads.Impl.COLUMN_MEDIAPROVIDER_URI,
            Downloads.Impl.COLUMN_DESTINATION, Downloads.Impl.COLUMN_TITLE, Downloads.Impl.COLUMN_DESCRIPTION, Downloads.Impl.COLUMN_URI,
            Downloads.Impl.COLUMN_STATUS, Downloads.Impl.COLUMN_FILE_NAME_HINT,
            ...
            Downloads.Impl.COLUMN_CURRENT_BYTES + " AS " + COLUMN_BYTES_DOWNLOADED_SO_FAR, Downloads.Impl.COLUMN_ALLOW_WRITE, "etag",
            /*
             * add the following 'computed' columns to the cursor. they are not 'returned' by the database, but their inclusion eliminates
             * need to have lot of methods in CursorTranslator
             */
     ...
     * 
     * Note that the default download destination is a shared volume where the system might delete your file if it needs to reclaim space
     * for system use. If this is a problem, use a location on external storage (see {@link #setDestinationUri(Uri)}.
     */
    public static class Request {
        ...
        /**
         * Bit flag for {@link #setAllowedNetworkTypes} corresponding to {@link ConnectivityManager#TYPE_BLUETOOTH}.
         * 
         * @hide
         */
        ...
        /**
         * if a file is designated as a MediaScanner scannable file, the following value is stored in the database column
        ...
        // MediaScanner
        /**
         * if a file is designated as a file that should not be scanned by MediaScanner, the following value is stored in the database
         * column {@link Downloads.Impl#COLUMN_MEDIA_SCANNED}.
         */
        ...
        /**
         * This download shows in the notifications after completion ONLY. It is usuable only with
         * {@link CLDownloadManager#addCompletedDownload(String, String, boolean, String, String, long, boolean)} .
         */
        public static final int VISIBILITY_VISIBLE_NOTIFY_ONLY_COMPLETION = 3;

        /**
         * can take any of the following values: {@link #VISIBILITY_HIDDEN} {@link #VISIBILITY_VISIBLE_NOTIFY_COMPLETED},
         * {@link #VISIBILITY_VISIBLE}, {@link #VISIBILITY_VISIBLE_NOTIFY_ONLY_COMPLETION}
         */
        ...
        /**
         * Set the local destination for the downloaded file. Must be a file URI to a path on external storage, and the calling application
         * must have the WRITE_EXTERNAL_STORAGE permission.
         * <p>
         * The downloaded file is not scanned by MediaScanner. But it can be made scannable by calling
         * {@link #allowScanningByMediaScanner()}.
         * <p>
         * By default, downloads are saved to a generated filename in the shared download cache and may be deleted by the system at any time
         * to reclaim space.
         * 
         * @return this object
         */
        ...
        /**
         * Set the local destination for the downloaded file to the system cache dir (/cache). This is only available to System apps with
         * the permission {@link android.Manifest.permission#ACCESS_CACHE_FILESYSTEM}.
         * <p>
         * The downloaded file is not scanned by MediaScanner. But it can be made scannable by calling
         * {@link #allowScanningByMediaScanner()}.
         * <p>
         ...
         * {@link CLDownloadManager#enqueue(Request)} is called.
         */
        ...
        /**
         * Set the title of this download, to be displayed in notifications (if enabled). If no title is given, a default one will be
        ...
        public Request setReferer(CharSequence referer) {
            mReferer = referer;
            return this;
        }
        ...
        /**
         * Control whether a system notification is posted by the download manager while this download is running. If enabled, the download
         * manager posts notifications about downloads through the system {@link android.app.NotificationManager}. By default, a
         ...
         * If enabled, the download manager posts notifications about downloads through the system {@link android.app.NotificationManager}.
         * By default, a notification is shown only when the download is in progress.
         * <p>
         * It can take the following values: {@link #VISIBILITY_HIDDEN}, {@link #VISIBILITY_VISIBLE},
         * {@link #VISIBILITY_VISIBLE_NOTIFY_COMPLETED}.
         * <p>
         * If set to {@link #VISIBILITY_HIDDEN}, this requires the permission android.permission.DOWNLOAD_WITHOUT_NOTIFICATION.
         * 
         * @param visibility
        ...
        /**
         * Restrict the types of networks over which this download may proceed. By default, all network types are allowed. Consider using
         * {@link #setAllowedOverMetered(boolean)} instead, since it's more flexible.
         * 
         * @param flags
        ...
        ContentValues toContentValues(String packageName) {
            ...
            if (mDestinationUri != null) {
                ...
                values.put(Downloads.Impl.COLUMN_DESTINATION, (this.mUseSystemCache) ? Downloads.Impl.DESTINATION_SYSTEMCACHE_PARTITION
    ...
    public static class Query {
        ...
        private String mLocalAbsFilePath;
        ...
        public Query setFilterByLocalAbsPath(String localAbsFilePath) {
            mLocalAbsFilePath = localAbsFilePath;
            return this;
        }
         ...
         * 
         * @param value
         *            if true, this query will only include downloads that should be displayed in the system's Downloads UI; if false (the
         *            default), this query will include both visible and invisible downloads.
         * @return this object
         * @hide
         */
         ...
         * 
         * @param column
         *            one of the COLUMN_* constants; currently, only {@link #COLUMN_LAST_MODIFIED_TIMESTAMP} and
        ...
        Cursor runQuery(ContentResolver resolver, String[] projection, Uri baseUri) {
            ...
            if (mUri != null) {
                selectionParts.add(Downloads.Impl.COLUMN_URI + " == '" + mUri + "'");
            }

            if (mLocalAbsFilePath != null) {
                selectionParts.add(Downloads.Impl._DATA + " == '" + mLocalAbsFilePath + "'");
            }
            ...
            if (DBG)
                Log.i(TAG, "runQuery. selection=" + selection);
    ...
    public static CLDownloadManager sDownloadManager;

    public static synchronized CLDownloadManager getInst(ContentResolver resolver, String packageName) {
        if (sDownloadManager == null) {
            sDownloadManager = new CLDownloadManager(resolver, packageName);
        }
     ...
     * @hide
     */
    private CLDownloadManager(ContentResolver resolver, String packageName) {
        mResolver = resolver;
        mPackageName = packageName;
    }

    /**
     * Makes this object access the download provider through /all_downloads URIs rather than /my_downloads URIs, for clients that have
     * permission to do so.
     * 
     * @hide
     */
    ...
    /**
     * Enqueue a new download. The download will start automatically once the download manager is ready to execute it and connectivity is
     * available.
     * 
     * @param request
    ...
    /**
     * Marks the specified download as 'to be deleted'. This is done when a completed download is to be removed but the row was stored
     * without enough info to delete the corresponding metadata from Mediaprovider database. Actual cleanup of this row is done in
     * DownloadService.
     * 
     * @param ids
    ...
    public int markRowDeletedInVsns(String[] vsnFileNames) {
        if (vsnFileNames == null || vsnFileNames.length == 0) {
            // called with nothing to remove!
            throw new IllegalArgumentException("input param 'vsnFileNames' can't be null");
        }
        ContentValues values = new ContentValues();
        values.put(Downloads.Impl.COLUMN_DELETED, 1);
        // if only one id is passed in, then include it in the uri itself.
        ...
        // COLUMN_DESCRIPTION is the vsn name.
        if (DBG)
            Log.i(TAG, "markRowDeletedInVsns. where=" + getWhereClauseForInNotInFiles(vsnFileNames, " IN ", Downloads.Impl.COLUMN_DESCRIPTION));

        return mResolver.update(mBaseUri, values, getWhereClauseForInNotInFiles(vsnFileNames, " IN ", Downloads.Impl.COLUMN_DESCRIPTION),
                vsnFileNames);
    }
    
    public int markRowDeletedNotInVsns(String[] vsnFileNames) {
        if (vsnFileNames == null || vsnFileNames.length == 0) {
            // called with nothing to remove!
            throw new IllegalArgumentException("input param 'vsnFileNames' can't be null");
        }
        ...
        // COLUMN_DESCRIPTION is the vsn name.
        final String notIn = " NOT IN ";
        if (DBG)
            Log.i(TAG, "markRowDeletedNotInVsns. where=" + getWhereClauseForInNotInFiles(vsnFileNames, notIn, Downloads.Impl.COLUMN_DESCRIPTION));
        
        return mResolver.update(mBaseUri, values, getWhereClauseForInNotInFiles(vsnFileNames, notIn, Downloads.Impl.COLUMN_DESCRIPTION),
                vsnFileNames);
    }

    public int markRowDeletedNotIn(String[] files) {
        if (files == null || files.length == 0) {
            // called with nothing to remove!
            throw new IllegalArgumentException("input param 'files' can't be null");
        }
        ContentValues values = new ContentValues();
        values.put(Downloads.Impl.COLUMN_DELETED, 1);
        // if only one id is passed in, then include it in the uri itself.
        ...
        if (DBG)
            Log.i(TAG, "markRowDeleted. where=" + getWhereClauseForInNotInFiles(files, " NOT IN ", Downloads.Impl._DATA));

        return mResolver.update(mBaseUri, values, getWhereClauseForInNotInFiles(files, " NOT IN ", Downloads.Impl._DATA), files);
    }

    static String getWhereClauseForInNotInFiles(String[] files, String inOrNotIn, String column) {
        StringBuilder whereClause = new StringBuilder();
        whereClause.append(column + inOrNotIn + " (");
        for (int i = 0; i < files.length; i++) {
            if (i > 0) {
                whereClause.append(" , ");
            }
            // XXX: Must not use the ', beside ?.
            whereClause.append(" ? ");
        }
        whereClause.append(")");
    ...
    public Cursor queryForFilesInDb(String[] absPaths) {
        return mResolver.query(mBaseUri, null, getWhereClauseForInNotInFiles(absPaths, " IN ", Downloads.Impl._DATA), absPaths, null);
    }

    public void resumeDownFiles(long[] ls) {
        if (DBG)
            Log.i(TAG, "resumeDownFiles. ids 0=" + ls[0]);

        // Start w/o precondition.
        ContentValues values = new ContentValues();
        ...
        mResolver.update(mBaseUri, values, getWhereClauseForIds(ls), getWhereArgsForIds(ls));

    }

    /**
     * Cancel downloads and remove them from the download manager. Each download will be stopped if it was running, and it will no longer be
     * accessible through the download manager. If there is a downloaded file, partial or complete, it is deleted.
     * 
     * @param ids
    ...
    /**
     * Returns the {@link Uri} of the given downloaded file id, if the file is downloaded successfully. Otherwise, null is returned.
     * <p>
     * If the specified downloaded file is in external storage (for example, /sdcard dir), then it is assumed to be safe for anyone to read
     * and the returned {@link Uri} corresponds to the filepath on sdcard.
     * 
     * @param id
     *            the id of the downloaded file.
     * @return the {@link Uri} of the given downloaded file id, if download was successful. null otherwise.
     */
    public Uri getUriForDownloadedFile(long id) {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                ...
                if (CLDownloadManager.STATUS_SUCCESSFUL == status) {
     ...
     * @param id
     *            the id of the downloaded file.
     * @return the media type of the given downloaded file id, if download was successful. null otherwise.
     */
    ...
    /**
     * Restart the given downloads, which must have already completed (successfully or not). This method will only work when called from
     * within the download manager's process.
     * 
     * @param ids
    ...
    public void restartDownload(long... ids) {
        ...
        // && Downloads.Impl.isStatusError(status)) {
        ...
        // }
        if (DBG)
            Log.i(TAG, "restartDownload. ids 0=" + ids[0]);
    ...
    // * there's no limit
    // *
    // * @param context the {@link Context} to use for accessing the {@link
    // ContentResolver}
    ...
    // /**
    // * Returns recommended maximum size, in bytes, of downloads that may go
    // over a mobile
    // * connection; or null if there's no recommended limit. The user will have
    // the option to bypass
    // * this limit.
    // *
    // * @param context the {@link Context} to use for accessing the {@link
    // ContentResolver}
    // * @return recommended maximum size, in bytes, of downloads that may go
    // over a mobile
    ...
    /**
     * Adds a file to the downloads database system, so it could appear in Downloads App (and thus become eligible for management by the
     * Downloads App).
     * <p>
     * It is helpful to make the file scannable by MediaScanner by setting the param isMediaScannerScannable to true. It makes the file
     * visible in media managing applications such as Gallery App, which could be a useful purpose of using this API.
     * 
     * @param title
     ...
     *            true if the file is to be scanned by MediaScanner. Files scanned by MediaScanner appear in the applications used to view
     *            media (for example, Gallery app).
     * @param mimeType
     *            mimetype of the file.
     * @param path
     *            absolute pathname to the file. The file should be world-readable, so that it can be managed by the Downloads App and any
     *            other app that is used to read it (for example, Gallery app to display the file, if the file contents represent a
     *            video/image).
     * @param length
     *            length of the downloaded file
     * @param showNotification
     *            true if a notification is to be sent, false otherwise
     * @return an ID for the download entry added to the downloads app, unique across the system This ID is used to make future calls
     *         related to this download.
     */
    public long addCompletedDownload(String title, String description, boolean isMediaScannerScannable, String mimeType, String path,
    ...
    public long addCompletedDownload(String title, String description, boolean isMediaScannerScannable, String mimeType, String path,
        ...
        values.put(Downloads.Impl.COLUMN_MEDIA_SCANNED, (isMediaScannerScannable) ? Request.SCANNABLE_VALUE_YES
                : Request.SCANNABLE_VALUE_NO);
        values.put(Downloads.Impl.COLUMN_VISIBILITY, (showNotification) ? Request.VISIBILITY_VISIBLE_NOTIFY_ONLY_COMPLETION
    ...
    public int pause(long[] ids) {
        if (ids == null || ids.length == 0) {
            // called with nothing to remove!
            throw new IllegalArgumentException("input param 'downingFiles' can't be null");
        }
        ContentValues values = new ContentValues();
    ...
    /**
     * This class wraps a cursor returned by DownloadProvider -- the "underlying cursor" -- and presents a different set of columns, those
     * defined in the DownloadManager.COLUMN_* constants. Some columns correspond directly to underlying values while others are computed
    ...
    private static class CursorTranslator extends CursorWrapper {
        ...
        private String getLocalUri() {
            ...
            if (destinationType == Downloads.Impl.DESTINATION_FILE_URI || destinationType == Downloads.Impl.DESTINATION_EXTERNAL
