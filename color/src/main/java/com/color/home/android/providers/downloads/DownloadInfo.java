...
import com.color.home.android.providers.downloads.Downloads.Impl;
...
public class DownloadInfo {
    ...
    public static class Reader {
        ...
        public DownloadInfo newDownloadInfo(Context context, SystemFacade systemFacade, CLStorageManager storageManager,
    ...
    private DownloadInfo(Context context, SystemFacade systemFacade, CLStorageManager storageManager, DownloadNotifier notifier) {
        mContext = context;
    ...
    public void sendIntentIfRequested() {
        ...
        Intent intent;
        if (mIsPublicApi) {
            intent = new Intent(CLDownloadManager.ACTION_DOWNLOAD_COMPLETE);
            intent.setPackage(mPackage);
            intent.putExtra(CLDownloadManager.EXTRA_DOWNLOAD_ID, mId);
    ...
    private boolean isReadyToDownload() {
        if (DBG)
            Log.i(TAG, "isReadyToDownload. mStatus=" + mStatus);
     ...
     * CLDownloadManager.Request.NETWORK_* bit flag.
     */
    private int translateNetworkTypeToApiFlag(int networkType) {
        switch (networkType) {
        case ConnectivityManager.TYPE_MOBILE:
            return CLDownloadManager.Request.NETWORK_MOBILE;

        case ConnectivityManager.TYPE_WIFI:
            return CLDownloadManager.Request.NETWORK_WIFI;

            // case ConnectivityManager.TYPE_BLUETOOTH:
            // return CLDownloadManager.Request.NETWORK_BLUETOOTH;
    ...
    public boolean shouldScanFile() {
        return (mMediaScanned == 0)
                && (mDestination == Downloads.Impl.DESTINATION_EXTERNAL || mDestination == Downloads.Impl.DESTINATION_FILE_URI || mDestination == Downloads.Impl.DESTINATION_NON_DOWNLOADMANAGER_DOWNLOAD)
                && Downloads.Impl.isStatusSuccess(mStatus);
    }

    void notifyPauseDueToSize(boolean isWifiRequired) {
        Log.e("DownloadInfo", "Error storage full.");
