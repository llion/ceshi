...
public class Helpers {
     ...
     * download.
     */
    static String generateSaveFile(Context context, String url, String hint, String contentDisposition, String contentLocation,
            String mimeType, int destination, long contentLength, CLStorageManager storageManager) throws StopRequestException {
        if (contentLength < 0) {
            contentLength = 0;
        }
        String path;
    ...
    static String getFullPath(String filename, String mimeType, int destination, File base) throws StopRequestException {
        ...
        synchronized (sUniqueLock) {
            final String path = chooseUniqueFilenameLocked(destination, filename, extension, recoveryDir);
            ...
            try {
                ...
                if (DBG)
                    Log.i(TAG, "getFullPath. f.getAbsolutePath()=" + file.getAbsolutePath() + ", getParentFile=" + file.getParentFile());
                File parentPath = file.getParentFile();
                if (!parentPath.exists()) {
                    if (DBG)
                        Log.i(TAG, "getFullPath. create as parentPath not exist=" + parentPath);
                    parentPath.mkdir();
                }
    ...
    private static String chooseUniqueFilenameLocked(int destination, String filename, String extension, boolean recoveryDir)
    ...
    static boolean isFilenameValid(String filename, File downloadsDataDir) {
        ...
        try {
            ...
            whitelist = new String[] { downloadsDataDir.getCanonicalPath(), Environment.getDownloadCacheDirectory().getCanonicalPath(),
    ...
    private static String replaceInvalidVfatCharacters(String filename) {
        ...
        for (int i = 0; i < filename.length(); i++) {
            ...
            if ((START_CTRLCODE <= ch && ch <= END_CTRLCODE) || ch == QUOTEDBL || ch == ASTERISK || ch == SLASH || ch == COLON
                    || ch == LESS || ch == GREATER || ch == QUESTION || ch == BACKSLASH || ch == BAR || ch == DEL) {
                if (!isRepetition) {
