...
public class DownloadService extends Service {
    ...
    private CLStorageManager mStorageManager;
    ...
    private static ExecutorService buildDownloadExecutor() {
        final int maxConcurrent = MAX_CONCURRENT;

        // Create a bounded thread pool for executing downloads; it creates
        // threads as needed (up to maximum) and reclaims them when finished.
        final ThreadPoolExecutor executor = new ThreadPoolExecutor(maxConcurrent, maxConcurrent, 10, TimeUnit.SECONDS,
    ...
    public void onCreate() {
        ...
        mStorageManager = new CLStorageManager(this);
    ...
    private void enqueueFinalUpdate() {
        ...
        mUpdateHandler.sendMessageDelayed(mUpdateHandler.obtainMessage(MSG_FINAL_UPDATE, mLastStartId, -1), 5 * MINUTE_IN_MILLIS);
    }
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                if (info.mDeleted) {
                    ...
                    if (DBG && (activeDownload || activeScan)) {
                        Log.v(TAG, "Download " + info.mId + ": activeDownload=" + activeDownload + ", activeScan=" + activeScan);
                    }
    ...
    private DownloadInfo insertDownloadLocked(DownloadInfo.Reader reader, long now) {
        ...
        if (Constants.LOGVV) {
            Log.v(TAG, "processing inserted download " + info.mId);
        }
    ...
    private void updateDownload(DownloadInfo.Reader reader, DownloadInfo info, long now) {
        ...
        if (Constants.LOGVV) {
            Log.v(TAG, "processing updated download " + info.mId + ", status: " + info.mStatus);
        }
    }
