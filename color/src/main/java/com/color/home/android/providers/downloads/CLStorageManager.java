...
import static com.color.home.android.providers.downloads.Constants.LOGV;
...
/**
 * Manages the storage space consumed by Downloads Data dir. When space falls below a threshold limit (set in resource xml files), starts
 * cleanup of the Downloads data dir to free up space.
 */
public class CLStorageManager {
    private static final boolean DBG = false;
    // never public, so that another class won't be messed up.
    private final static String TAG = "CLStorageManager";
    /** the max amount of space allowed to be taken up by the downloads data dir */
    private static final long sMaxdownloadDataDirSize = 14 * 1024 * 1024 * 1024;

    /**
     * threshold (in bytes) beyond which the low space warning kicks in and attempt is made to purge some downloaded files to make space
     */
    private static final long sDownloadDataDirLowSpaceThreshold = 50 * 1024 * 1024;
    ...
    /**
     * The downloaded files are saved to this dir. it is the value returned by {@link Context#getCacheDir()}.
     */
    ...
    public CLStorageManager(Context context) {
        if (DBG)
            Log.i(TAG, "CLStorageManager. context");
        mContext = context;
    ...
    /**
     * How often should database and filesystem be cleaned up to remove spurious files from the file system and The value is specified in
     * terms of num of downloads since last time the cleanup was done.
     */
    ...
    /*
     * start a thread to cleanup the following remove spurious files from the file system remove excess entries from the database
     */
    ...
    void verifySpace(int destination, String path, long length) throws StopRequestException {
        ...
        File dir = null;
        if (Constants.LOGV) {
            Log.i(Constants.TAG, "in verifySpace, destination: " + destination + ", path: " + path + ", length: " + length);
        }
        if (path == null) {
    ...
    /**
     * finds space in the given filesystem (input param: root) to accommodate # of bytes specified by the input param(targetBytes). returns
    ...
    private synchronized void findSpace(File root, long targetBytes, int destination) throws StopRequestException {
        ...
        if (destination == Downloads.Impl.DESTINATION_FILE_URI || destination == Downloads.Impl.DESTINATION_EXTERNAL) {
        ...
        if (bytesAvailable < sDownloadDataDirLowSpaceThreshold) {
            /*
             * filesystem's available space is below threshold for low space warning. threshold typically is 10% of download data dir space
             * quota. try to cleanup and see if the low space situation goes away.
             */
            ...
            if (bytesAvailable < sDownloadDataDirLowSpaceThreshold) {
                 ...
                 * 
                 * If this is system cache dir, print a warning. otherwise, don't allow downloading until more space is available because
                 * downloadmanager shouldn't end up taking those last few MB of space left on the filesystem.
                 */
                if (root.equals(mSystemCacheDir)) {
                    Log.w(Constants.TAG, "System cache dir ('/cache') is running low on space." + "space available (in bytes): "
                            + bytesAvailable);
                } else {
                    throw new StopRequestException(Downloads.Impl.STATUS_INSUFFICIENT_SPACE_ERROR, "space in the filesystem rooted at: "
                            + root + " is below 10% availability. stopping this download.");
                }
            }
        }
        if (root.equals(mDownloadDataDir)) {
            ...
            if (bytesAvailable < sDownloadDataDirLowSpaceThreshold) {
                // print a warning
                Log.w(Constants.TAG, "Downloads data dir: " + root + " is running low on space. space available (in bytes): "
                        + bytesAvailable);
            }
    ...
    /**
     * returns the number of bytes available in the downloads data dir TODO this implementation is too slow. optimize it.
     */
    ...
    File locateDestinationDirectory(String mimeType, int destination, long contentLength) throws StopRequestException {
        ...
        case Downloads.Impl.DESTINATION_EXTERNAL:
            ...
            if (!base.isDirectory() && !base.mkdir()) {
                ...
                // "download"
                // already exists at the root level, or the SD card filesystem
                // is read-only.
                throw new StopRequestException(Downloads.Impl.STATUS_FILE_ERROR, "unable to create external downloads directory "
                        + base.getPath());
            }
            return base;
        default:
    ...
    public static File getDownloadDataDirectory(Context context) {
        if (Constants.LOGV)
            Log.d(TAG,
                    "download context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS) dir="
                            + context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS));
        // return new File(MainActivity.FOLDER_PLAY_NETWORK);
        return context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS);
    }

    /**
     * Deletes purgeable files from the cache partition. This also deletes the matching database entries. Files are deleted in LRU order
     * until the total byte size is greater than targetBytes
     */
    private long discardPurgeableFiles(int destination, long targetBytes) {
        if (Constants.LOGV) {
            Log.i(Constants.TAG, "discardPurgeableFiles: destination = " + destination + ", targetBytes = " + targetBytes);
        }
        String destStr = (destination == Downloads.Impl.DESTINATION_SYSTEMCACHE_PARTITION) ? String.valueOf(destination) : String
                .valueOf(Downloads.Impl.DESTINATION_CACHE_PARTITION_PURGEABLE);
        String[] bindArgs = new String[] { destStr };
        Cursor cursor = mContext.getContentResolver().query(
                Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI,
                null,
                "( " + Downloads.Impl.COLUMN_STATUS + " = '" + Downloads.Impl.STATUS_SUCCESS + "' AND " + Downloads.Impl.COLUMN_DESTINATION
                        + " = ? )", bindArgs, Downloads.Impl.COLUMN_LAST_MODIFICATION);
        if (cursor == null) {
            return 0;
        }
        long totalFreed = 0;
        ...
        if (true || Constants.LOGV) {
            Log.i(Constants.TAG, "Purged files, freed " + totalFreed + " for " + targetBytes + " requested");
        }
        return totalFreed;
    }

    /**
     * Removes files in the systemcache and downloads data dir without corresponding entries in the downloads database. This can occur if a
     * delete is done on the database but the file is not removed from the filesystem (due to sudden death of the process, for example).
    ...
    private void removeSpuriousFiles() {
        ...
        // Exclude the playing vsn, because we copied it from another file.
        // it's not in the db.
        // String playingVsnFile = PlayingNetPrograms.getSharedPreferences(mContext).getString(PlayingNetPrograms.KEY_LOCAL_URI, "");
        // if (!TextUtils.isEmpty(playingVsnFile)) {
        // files.remove(new File(playingVsnFile));
        // }
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    if (!TextUtils.isEmpty(filename)) {
                        if (LOGV) {
                            Log.i(Constants.TAG, "in removeSpuriousFiles, preserving file " + filename);
                        }
        ...
        for (File file : files) {
            ...
            if (path.endsWith(".vsn") || path.endsWith(".json")) {
                if (DBG)
                    Log.i(TAG, "removeSpuriousFiles. forgive the .vsn and .json file. path=" + path);
            } else {

                // final StructStat stat = Libcore.os.stat(path);
                // if (stat.st_uid == myUid) {
                if (Constants.LOGVV)
                    Log.d(TAG, "try to delete spurious file " + path);
                file.delete();
            }
        }
    }
    ...
    /**
     * Drops old rows from the database to prevent it from growing too large TODO logic in this method needs to be optimized. maintain the
     * number of downloads in memory - so that this method can limit the amount of data read.
     */
    private void trimDatabase() {
        ...
        Cursor cursor = null;
        try {
            cursor = mContext.getContentResolver().query(Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI, new String[] { Downloads.Impl._ID },
            ...
            if (cursor.moveToFirst()) {
                ...
                while (numDelete > 0) {
                    Uri downloadUri = ContentUris.withAppendedId(Downloads.Impl.ALL_DOWNLOADS_CONTENT_URI, cursor.getLong(columnId));
